...
.NOTES
    ...
    Uninstall added 15/10/2020 Mark.Burns@dell.com -Uninstall
#>
...
param(
    ...
    [parameter(Mandatory=$false, HelpMessage="Uninstall apps")]
...
Process {
    ...
    function Install-ApplicationExectuable {
            ...
            foreach ($ApplicationExecutableFolder in $ApplicationExecutableFolders) {
                ...
                if (($CurrentApplicationPath | Measure-Object).Count -eq 1) {
                    ...
                    if (($CurrentApplicationPath -ne $null) -and ([System.IO.Path]::GetExtension((Split-Path -Path $CurrentApplicationPath -Leaf)) -like ".exe")) {
                        ...
                        Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Installing: $($CurrentFileDescription)"
                        ...
                        if (($ReturnValue.ExitCode -eq 0) -or ($ReturnValue.ExitCode -eq 3010)) {
                            ...
                            Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Successfully installed: $($CurrentFileDescription)"
                        }
                        else {
                            Write-Verbose -Message "Failed to install: $($CurrentFileDescription)"
                            Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Failed to install: $($CurrentFileDescription)"
                        }
                    }
                    else {
        ...
        function Uninstall-ApplicationExecutable {
        param(
            [parameter(Mandatory=$true)]
            [ValidateNotNullOrEmpty()]
            [ValidateSet("x86","x64")]
        ...
        if ($ApplicationExecutableFolders -ne $null) {
            ...
            foreach ($ApplicationExecutableFolder in $ApplicationExecutableFolders) {
                ...
                if (($CurrentApplicationPath | Measure-Object).Count -eq 1) {
                    ...
                    if (($CurrentApplicationPath -ne $null) -and ([System.IO.Path]::GetExtension((Split-Path -Path $CurrentApplicationPath -Leaf)) -like ".exe")) {
                        ...
                        if ($Script:PSBoundParameters["ShowProgress"]) {
                            $ProgressCount++
                            Write-Progress -Activity "Uninstalling Microsoft Visual C++ Redistributables ($($ApplicationArchitecture))" -Id 1 -Status "$($ProgressCount) / $($ApplicationExecutableFolderCount)" -CurrentOperation "Installing: $($CurrentFileDescription)" -PercentComplete (($ProgressCount / $ApplicationExecutableFolderCount) * 100)
                        }
                        Write-Verbose -Message "Uninstalling: $($CurrentFileDescription)"
                        Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Uninstalling: $($CurrentFileDescription)"
                        # Install the current executable
                        $ReturnValue = Start-Process -FilePath $CurrentApplicationPath -ArgumentList "/q /uninstall /norestart" -Wait -PassThru
                        if (($ReturnValue.ExitCode -eq 0) -or ($ReturnValue.ExitCode -eq 3010)) {
                            Write-Verbose -Message "Successfully uninstalled: $($CurrentFileDescription)"
                            Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Successfully uninstalled: $($CurrentFileDescription)"
                        }
                        else {
                            Write-Verbose -Message "Failed to uninstall: $($CurrentFileDescription)"
                            Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Failed to uninstall: $($CurrentFileDescription)"
                        }
                    }
                    else {
                        Write-Warning -Message "Unsupported file extension found in folder: $($ApplicationExecutableFolder)"
            ...
            if ($Script:PSBoundParameters["ShowProgress"]) {
                Write-Progress -Activity "Uninstalling Microsoft Visual C++ Redistributables ($($ApplicationArchitecture))" -Id 1 -Completed -Status "Completed"
            }
        }
    }
    $TimeStamp = "[{0:MM/dd/yy} {0:HH:mm:ss}]" -f (Get-Date)
    Add-Content "C:\Windows\Logs\Install-VisualCRedist.log" "Install-VisualCRedist $($TimeStamp)"
    if ($Script:PSBoundParameters["Uninstall"]) {
         # Uninstall Microsoft Visual C++ Redistributables
        foreach ($Arch in $Architecture) {
            Uninstall-ApplicationExecutable -ApplicationArchitecture $Arch -CurrentWorkingDirectory (Join-Path -Path (Split-Path -Path $MyInvocation.MyCommand.Definition -Parent) -ChildPath "\Source")
        }
